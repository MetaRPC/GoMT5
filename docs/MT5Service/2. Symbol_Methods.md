...
- ✅ One GetSymbolParamsMany call instead of 10+ SymbolInfo* calls
- ✅ Returns clean Go types (float64, int64, string)
- ✅ SymbolTick with time.Time (not Unix timestamp)
- ✅ SessionTime with time.Time (not protobuf Timestamp)
...
**Code reduction: 60%** (10 lines → 4 lines)
...
**Code reduction: 70%** (30+ lines → 9 lines)
...
### ✅ GetSymbolParamsMany (RECOMMENDED)
...
### ✅ GetSymbolDouble/Integer/String
...
### ✅ SymbolExist / SymbolSelect / IsSymbolSynchronized
...
1. **For multiple properties** → use `GetSymbolParamsMany` instead of multiple `GetSymbolDouble/Integer` calls
2. **For current price** → use `GetSymbolTick` (Bid/Ask + timestamp in one call)
3. **Before trading** → check `SymbolExist` + `IsSymbolSynchronized`
4. **For batch operations** → use `GetSymbolParamsMany` with pagination
5. **For filtering** → get all symbols via `GetSymbolParamsMany`, then filter in code
...
- [Account Information (Low-Level)](../API_Reference/MT5Account.md) - low-level account API
- [Trading Methods (Mid-Level)](./4.%20Trading_Methods.md) - trading methods
- [Trading Operations (Low-Level)](../API_Reference/MT5Account.md) - low-level trading API
- [Streaming Methods (Mid-Level)](./6.%20Streaming_Methods.md) - tick streaming
- [Streaming Methods (Low-Level)](../API_Reference/MT5Account.md) - low-level streaming API
- [MT5Account Master Overview](../API_Reference/MT5Account.md) - low-level API master reference
...
- ✅ Get all symbol parameters in one `GetSymbolParamsMany` call
- ✅ SymbolTick and SessionTime with ready `time.Time`
- ✅ Code reads like normal Go
