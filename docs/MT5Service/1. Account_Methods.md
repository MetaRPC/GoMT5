...
- ✅ Less code (1-2 lines instead of 3-4)
- ✅ No need to call `.GetRequestedValue()`
- ✅ Returns clean Go types (`float64`, `int64`, `string`)
- ✅ Automatic creation of request structures
- ✅ Conversion `*timestamppb.Timestamp` → `*time.Time` (in GetAccountSummary)
...
**Code reduction: 50%** (10 lines → 5 lines)
...
```go
// ✅ BEST way - use GetAccountSummary for multiple properties
...
### ✅ GetAccountDouble / GetAccountInteger / GetAccountString
...
1. **For a single property** → use `GetAccountDouble/Integer/String`
2. **For 2-3 properties** → use `GetAccountSummary` (already has everything)
3. **For all information** → use `GetAccountSummary`
...
- ✅ Get clean Go types (`float64`, `int64`, `string`, `*AccountSummary`)
- ✅ Code reads like normal Go, without protobuf dependencies
